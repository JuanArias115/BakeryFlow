...
namespace BakeryFlow.Application.Features.Categories;
...
public sealed class CategoryService(IBakeryFlowDbContext dbContext) : ICategoryService
{
    public async Task<PagedResult<CategoryDto>> GetPagedAsync(PagedRequest request, CancellationToken cancellationToken = default)
    {
        var query = dbContext.Categories
            .AsNoTracking()
            .Where(x => string.IsNullOrWhiteSpace(request.Search) || x.Name.ToLower().Contains(request.Search.ToLower()))
            .OrderBy(x => x.Name)
            .Select(x => new CategoryDto(x.Id, x.Name, x.Description, x.IsActive, x.CreatedAt, x.UpdatedAt));
    ...
    {
        var exists = await dbContext.Categories.AnyAsync(x => x.Name.ToLower() == request.Name.Trim().ToLower(), cancellationToken);
        if (exists)
        {
        ...
        return new CategoryDto(category.Id, category.Name, category.Description, category.IsActive, category.CreatedAt, category.UpdatedAt);
    }
    ...
    public async Task<CategoryDto> UpdateAsync(Guid id, SaveCategoryRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var exists = await dbContext.Categories.AnyAsync(
            x => x.Id != id && x.Name.ToLower() == request.Name.Trim().ToLower(),
            cancellationToken);
